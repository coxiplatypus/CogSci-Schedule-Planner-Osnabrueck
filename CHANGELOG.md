...
- ECTS panel rows now show both *total LP* (past imported + this semester) and *this semester LP* in every area, Free Elective, Study Project and Colloquium row, separated by a `│` divider and labelled by a column header above the list. Equally prominent so neither number gets lost. Header and rows share a single CSS grid so every `│` divider lines up in a single column.
- ECTS progress bars gained a white past│sem divider inside the bar, and the spec (20 LP) and cap (32 LP) markers moved **out** of the bar as small green / red triangles pointing up at it from a 5 px strip below. This removes the collision between the divider line and the spec line that happened whenever imported past credits equalled exactly 20 LP. Every segment and marker has an instant custom tooltip on hover (no browser-title delay) showing the exact LP value.
- ECTS summary boxes: the left "Compulsory Electives / 44" box now shows **total** LP (past imported + this-semester locked/likely) against the lifetime target, instead of only this-semester LP. The right box still shows this-semester LP. Each box gained a tiny uppercase `total` / `this sem` tag above the big number and an explanatory tooltip so it's always obvious which number is which.
