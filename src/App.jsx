...
function App(){
  ...
  const [pres,setPres]=useState(()=>{const o={};C.forEach(c=>{c.slots.forEach((_,si)=>{o[`${c.id}:${si}`]="unclear"})});return o});
  ...
  const [presMenu,setPresMenu]=useState(null); // {id,si,x,y} — attendance menu on grid right-click
  const [barTip,setBarTip]=useState(null); // {text,x,y} — instant tooltip for ECTS bar segments

  const isOn=(id)=>!!status[id]&&status[id]!=="dropped";
  const stOf=(id)=>ST.find(s=>s.v===status[id])||null;
  const presOf=(id,si)=>pres[`${id}:${si}`]||"unclear";
  const cfg=CFGS[ck];
  ...
  useEffect(()=>{
    ...
    if(d){
      ...
      if(d.pres){
        const isOld=Object.keys(d.pres).some(k=>!k.includes(":"));
        if(isOld){
          const allC=[...C,...(d.custom||[])];
          const np={};
          Object.entries(d.pres).forEach(([id,v])=>{
            const c=allC.find(x=>x.id===id);
            if(c)c.slots.forEach((_,si)=>{np[`${id}:${si}`]=v});
          });
          setPres(p=>({...p,...np}));
        }else{
          setPres(p=>({...p,...d.pres}));
      ...
      if(d.custom){setCustom(d.custom);d.custom.forEach(c=>{c.slots.forEach((_,si)=>{setPres(p=>({...p,[`${c.id}:${si}`]:p[`${c.id}:${si}`]||"unclear"}))})})}
  ...
  const cyclePres=(id,si)=>{const k=`${id}:${si}`;const o=["yes","unclear","no"];setPres(p=>({...p,[k]:o[(o.indexOf(p[k]||"unclear")+1)%3]}))};
  const cyclePresAll=(id)=>{const o=["yes","unclear","no"];setPres(p=>{const n={...p};const crs=courses.find(x=>x.id===id);if(!crs)return n;const vals=crs.slots.map((_,si)=>n[`${id}:${si}`]||"unclear");const cur=vals.includes("yes")?"yes":vals.includes("unclear")?"unclear":"no";const next=o[(o.indexOf(cur)+1)%3];crs.slots.forEach((_,si)=>{n[`${id}:${si}`]=next});return n})};
  ...
  const addCustom=()=>{
    ...
    const nc={id,name:f.name.trim(),short:f.short.trim()||f.name.trim().split(/\s+/)[0],ects:parseInt(f.ects)||4,nr:"",
      deg:"both",cat:"course",a24:f.a24,a19:f.a19,room:f.room||"",slots:addSlots.map(s=>({day:parseInt(s.day),start:parseTime(s.start),end:parseTime(s.end),...(s.room?{room:s.room}:{})})),note:"Custom",isCustom:true};
    setCustom(p=>[...p,nc]);
    setStatus(p=>({...p,[id]:"considering"}));
    nc.slots.forEach((_,si)=>{setPres(p=>({...p,[`${id}:${si}`]:"unclear"}))});
    setNextCid(n=>n+1);
  ...
  const removeCustom=(id)=>{
    ...
    setPres(p=>{const n={...p};Object.keys(n).filter(k=>k.startsWith(id+":")).forEach(k=>delete n[k]);return n});
  };
  ...
  const startEdit=(c)=>{
    ...
    setEditSlots(c.slots.map(s=>({day:s.day,start:parseTime(s.start),end:parseTime(s.end),label:s.label||"",room:s.room||""})));
  };
  const saveEdit=()=>{
    if(!editForm||!editingId)return;
    const f=editForm;
    const patch={name:f.name.trim(),short:f.short.trim(),ects:parseInt(f.ects)||4,a24:f.a24,a19:f.a19,nr:f.nr,room:f.room.trim(),
      slots:editSlots.map(s=>({day:parseInt(s.day),start:parseTime(s.start),end:parseTime(s.end),...(s.label?{label:s.label}:{}),...(s.room?{room:s.room}:{})}))};
  ...
  // Per-slot room edit (from right-click menu). Writes to the edits overlay for
  // built-in courses and directly into the custom array for user-added ones.
  const updateSlotRoom=(courseId,slotIdx,room)=>{
    const c=courses.find(x=>x.id===courseId);if(!c)return;
    const trimmed=(room||"").trim();
    const curSlot=c.slots[slotIdx]||{};
    if((curSlot.room||"")===trimmed)return;
    const newSlots=c.slots.map((s,i)=>{
      if(i!==slotIdx)return s;
      const {room:_,...rest}=s;
      return trimmed?{...rest,room:trimmed}:rest;
    });
    if(courseId.startsWith("custom-")){
      setCustom(p=>p.map(x=>x.id===courseId?{...x,slots:newSlots}:x));
    }else{
      setEdits(p=>{
        const prev=p[courseId]||{};
        return {...p,[courseId]:{...prev,slots:newSlots}};
      });
    }
  };
  ...
  const conflicts=useMemo(()=>{
    ...
    for(let i=0;i<act.length;i++)for(let j=i+1;j<act.length;j++){
      for(const [sai,sa] of act[i].slots.entries())for(const [sbi,sb] of act[j].slots.entries()){
        if(!overlaps(sa,sb))continue;
        if(presOf(act[i].id,sai)==="no"||presOf(act[j].id,sbi)==="no")continue;
  ...
  const wpBase=cfg.areas.reduce((s,a)=>s+(baseline[a.key]||0),0);
  const wpGrandTotal=wpBase+wpTotal; // past + this semester confirmed, for the /44 lifetime target
  ...
  // ── Grid slot map: each entry is placed in the hour-cell of its start,
  // using fractional hours for sub-hour offsets and heights. Overlap lanes are
  // computed per integer-hour cell so that courses touching the same row-cell
  // render side-by-side. ──
  const slotMap=useMemo(()=>{
    const m={};DAYS.forEach((_,di)=>HOURS.forEach(h=>{m[`${di}-${h}`]=[]}));
    visible.forEach(c=>{if(!isOn(c.id))return;
      c.slots.forEach((sl,si)=>{
        const s=parseTime(sl.start),e=parseTime(sl.end);
        const startH=Math.floor(s),endH=Math.ceil(e);
        // Register as a starter in its start hour cell
        const sk=`${sl.day}-${startH}`;
        if(m[sk])m[sk].push({course:c,slot:sl,si,isStart:true,startDec:s,endDec:e});
        // Register as an occupier in every subsequent hour cell so overlap-lane
        // computation knows the cell is busy, but without a second rendered block.
        for(let h=startH+1;h<endH;h++){
          const k=`${sl.day}-${h}`;
          if(m[k])m[k].push({course:c,slot:sl,si,isStart:false,startDec:s,endDec:e});
        }
      });
    });return m;
  },[status,visible]);
  ...
  const reset=()=>{setStatus({});const p={};courses.forEach(c=>{c.slots.forEach((_,si)=>{p[`${c.id}:${si}`]="unclear"})});setPres(p)};
  const deleteAllData=()=>{
    try{localStorage.removeItem(SK)}catch(e){}
    setStatus({});setEdits({});setBaseline({});setCustom([]);
    const p={};C.forEach(c=>{c.slots.forEach((_,si)=>{p[`${c.id}:${si}`]="unclear"})});setPres(p);
  ...
  // Slot editor: a render function (NOT a component) so its <input>s stay mounted
  // across parent re-renders. Defining it as a component inside App gave every
  // render a fresh function identity, which React treats as a new type → unmount
  // → remount → focus loss after every keystroke in room/time fields.
  const renderSlotEditor=(slots,setSlots)=>(
    <div style={{marginBottom:3}}>
      {slots.map((sl,i)=>(
        <div key={i} style={{display:"flex",gap:3,marginBottom:2,alignItems:"center"}}>
          <select value={sl.day} onChange={e=>{const n=[...slots];n[i]={...n[i],day:parseInt(e.target.value)};setSlots(n)}} style={{flex:1,padding:"3px 5px",borderRadius:4,border:"1px solid #d1d5db",fontSize:12}}>
            {DAYS.map((d,di)=><option key={di} value={di}>{d}</option>)}
          </select>
          <input type="time" step="900" value={fmtTime(sl.start)} onChange={e=>{const n=[...slots];n[i]={...n[i],start:parseTime(e.target.value)};setSlots(n)}} style={{width:82,padding:"3px",borderRadius:4,border:"1px solid #d1d5db",fontSize:12}} title="Start time (15-min steps; type any minute)"/>
          <span style={{fontSize:9,color:"#9ca3af"}}>–</span>
          <input type="time" step="900" value={fmtTime(sl.end)} onChange={e=>{const n=[...slots];n[i]={...n[i],end:parseTime(e.target.value)};setSlots(n)}} style={{width:82,padding:"3px",borderRadius:4,border:"1px solid #d1d5db",fontSize:12}} title="End time (15-min steps; type any minute)"/>
      ...
      <button onClick={()=>setSlots([...slots,{...DEFAULT_SLOT}])} style={{fontSize:12,color:"#2563eb",background:"none",border:"none",padding:"2px 0"}}>+ Add time slot</button>
    </div>
  );

  // Render a course row in sidebar — render function (NOT a component) so the
  // inline edit form's <input>s don't get unmounted on every keystroke.
  const renderCourseRow=(c,g)=>{
    ...
    const prVals=c.slots.map((_,si)=>presOf(c.id,si));
    const summaryPres=prVals.includes("yes")?"yes":prVals.includes("unclear")?"unclear":"no";
    const pr=PR.find(p=>p.v===summaryPres);
    const isEdited=!!edits[c.id];
    const isHovered=hov===c.id;

    // Inline edit form
    if(editingId===c.id)return(
      <div key={c.id+"-edit"} style={{padding:6,background:"#f8fafc",borderRadius:5,marginBottom:2,border:"1px solid #e5e7eb"}}>
        ...
        <input value={editForm.room} onChange={e=>setEditForm(p=>({...p,room:e.target.value}))} placeholder="Default room (optional — per-slot overrides below)" style={{width:"100%",padding:"3px 5px",borderRadius:4,border:"1px solid #d1d5db",fontSize:12,marginBottom:3}}/>
        {renderSlotEditor(editSlots,setEditSlots)}
    ...
    return(
      <div key={c.id} style={{
        ...
        {active&&hasSched&&!isDropped&&<button onClick={()=>cyclePresAll(c.id)} title={pr?`${pr.l} — click to change all slots`:""} style={{
  ...
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",margin:"0 auto",padding:"10px 24px",overflow:"hidden"}}>
      ...
      <div className="app-flex" style={{display:"flex",flex:1,overflow:"hidden",minHeight:0}}>
        ...
        <div className="app-sidebar" style={{flex:`0 0 ${isMobile?"100%":sideW+"px"}`,minWidth:isMobile?"auto":200,display:"flex",flexDirection:"column",height:isMobile?(mobileGridH?"auto":"40vh"):"100%",overflow:"hidden"}}>
          ...
          <div data-ects="1" style={{background:"white",border:"1px solid #e5e7eb",borderRadius:10,padding:11,marginBottom:0,overflow:"auto",position:"relative",...(ectsH?{height:ectsH,flexShrink:0}:{flexShrink:0})}}>
            {/* Single grid wraps header + area rows + FE/SP/Colloquium rows so every
                divider '│' lines up in one column across the whole panel. Bars and tick
                strips span all 4 columns via gridColumn:"1 / -1". */}
            <div style={{display:"grid",gridTemplateColumns:"1fr auto 10px auto",alignItems:"baseline",columnGap:0,rowGap:0}}>
              <div style={{fontSize:13,fontWeight:700,gridColumn:1,marginBottom:4}}>ECTS</div>
              <div title="Left column: total LP (past imported + this semester locked/likely). Right column: this semester only. Purple (+N?) = considering this semester." style={{gridColumn:2,textAlign:"right",fontSize:10,color:"#6b7280",fontWeight:600,letterSpacing:".2px",marginBottom:4}}>total</div>
              <div style={{gridColumn:3,textAlign:"center",fontSize:10,color:"#d1d5db",fontWeight:600,marginBottom:4}}>│</div>
              <div style={{gridColumn:4,fontSize:10,color:"#6b7280",fontWeight:600,letterSpacing:".2px",marginBottom:4}}>sem LP</div>
              {cfg.areas.map(a=>{
                ...
                const sel=lk+ly;
                const numColor=over?"#dc2626":spec?"#059669":"#4b5563";
                return(<React.Fragment key={a.key}>
                  <div style={{gridColumn:1,color:a.cl.t,fontSize:12.5,fontWeight:600,marginTop:3}}>{a.key}</div>
                  <div style={{gridColumn:2,textAlign:"right",fontSize:12.5,fontWeight:600,color:numColor,marginTop:3}}>{v}</div>
                  <div style={{gridColumn:3,textAlign:"center",fontSize:12.5,fontWeight:400,color:"#d1d5db",marginTop:3}}>│</div>
                  <div style={{gridColumn:4,fontSize:12.5,fontWeight:600,color:numColor,marginTop:3}}>{sel}{cv>0?<span style={{color:"#8b5cf6"}}>{` (+${cv}?)`}</span>:""}{spec?" ✓":""}{over?` ⚠>${a.cap}`:""}</div>
                  <div style={{gridColumn:"1 / -1",marginTop:2}}>
                    <div style={{height:7,background:"#e5e7eb",borderRadius:4,overflow:"hidden",position:"relative"}}>
                      {bl>0&&<div onMouseEnter={e=>setBarTip({text:`Past (imported): ${bl} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:0,top:0,height:"100%",width:`${Math.min(100,(bl/mx)*100)}%`,background:a.cl.c,opacity:.75,transition:"width .2s"}}/>}
                      {lk>0&&<div onMouseEnter={e=>setBarTip({text:`Locked this semester: ${lk} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:`${Math.min(100,(bl/mx)*100)}%`,top:0,height:"100%",width:`${Math.min(100,(lk/mx)*100)}%`,background:a.cl.c,opacity:.55,transition:"width .2s"}}/>}
                      {ly>0&&<div onMouseEnter={e=>setBarTip({text:`Likely this semester: ${ly} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:`${Math.min(100,((bl+lk)/mx)*100)}%`,top:0,height:"100%",width:`${Math.min(100,(ly/mx)*100)}%`,background:a.cl.c,opacity:.3,transition:"width .2s"}}/>}
                      {cv>0&&<div onMouseEnter={e=>setBarTip({text:`Considering this semester: ${cv} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:`${Math.min(100,(v/mx)*100)}%`,top:0,height:"100%",width:`${Math.min(100-Math.min(100,(v/mx)*100),(cv/mx)*100)}%`,background:a.cl.c,opacity:.15,transition:"width .2s"}}/>}
                      {bl>0&&(lk+ly+cv)>0&&<div style={{position:"absolute",left:`${(bl/mx)*100}%`,top:0,height:"100%",width:2,background:"white",boxShadow:"0 0 0 .5px rgba(17,24,39,.5)",pointerEvents:"none"}}/>}
                    </div>
                    {(a.spec||(a.cap&&a.cap<900))&&<div style={{height:5,position:"relative",marginTop:1}}>
                      {a.spec&&<div onMouseEnter={e=>setBarTip({text:`Specialization threshold: ${a.spec} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:`${(a.spec/mx)*100}%`,top:0,marginLeft:-5,width:10,height:5}}><div style={{marginLeft:2,width:0,height:0,borderLeft:"3px solid transparent",borderRight:"3px solid transparent",borderBottom:"5px solid #059669"}}/></div>}
                      {a.cap&&a.cap<900&&<div onMouseEnter={e=>setBarTip({text:`Cap: ${a.cap} LP`,x:e.clientX,y:e.clientY})} onMouseMove={e=>setBarTip(p=>p?{...p,x:e.clientX,y:e.clientY}:p)} onMouseLeave={()=>setBarTip(null)} style={{position:"absolute",left:`${(a.cap/mx)*100}%`,top:0,marginLeft:-5,width:10,height:5}}><div style={{marginLeft:2,width:0,height:0,borderLeft:"3px solid transparent",borderRight:"3px solid transparent",borderBottom:"5px solid #dc2626"}}/></div>}
                    </div>}
                  </div>
                </React.Fragment>);
              })}
              {(()=>{const feBl=baseline[cfg.fe.key]||0;const feV=feBl+feTotal;return(<React.Fragment key="fe-row">
                <div style={{gridColumn:1,fontSize:12.5,color:"#4b5563",marginTop:4}}>Free Elective</div>
                <div style={{gridColumn:2,textAlign:"right",fontSize:12.5,color:"#4b5563",marginTop:4}}>{feV}</div>
                <div style={{gridColumn:3,textAlign:"center",fontSize:12.5,color:"#d1d5db",marginTop:4}}>│</div>
                <div style={{gridColumn:4,fontSize:12.5,color:"#4b5563",marginTop:4}}>{feTotal}{feCons>0?<span style={{color:"#8b5cf6"}}>{` (+${feCons}?)`}</span>:""}{cfg.feN?` / ${cfg.feN}`:""}</div>
              </React.Fragment>)})()}
              {isMsc&&(()=>{const spBl=baseline["Study Project"]||0;const spSem=(proj.locked["Study Project"]||0)+(proj.likely["Study Project"]||0);const spCv=proj.considering["Study Project"]||0;return(<React.Fragment key="sp-row">
                <div style={{gridColumn:1,fontSize:12.5,color:"#4b5563"}}>Study Project</div>
                <div style={{gridColumn:2,textAlign:"right",fontSize:12.5,color:"#4b5563"}}>{spBl+spSem}</div>
                <div style={{gridColumn:3,textAlign:"center",fontSize:12.5,color:"#d1d5db"}}>│</div>
                <div style={{gridColumn:4,fontSize:12.5,color:"#4b5563"}}>{spSem}{spCv>0?<span style={{color:"#8b5cf6"}}>{` (+${spCv}?)`}</span>:""}</div>
              </React.Fragment>)})()}
              {(()=>{const coBl=baseline["Colloquium"]||0;const coSem=(proj.locked["Colloquium"]||0)+(proj.likely["Colloquium"]||0);const coCv=proj.considering["Colloquium"]||0;return(<React.Fragment key="co-row">
                <div style={{gridColumn:1,fontSize:12.5,color:"#4b5563"}}>Colloquium</div>
                <div style={{gridColumn:2,textAlign:"right",fontSize:12.5,color:"#4b5563"}}>{coBl+coSem}</div>
                <div style={{gridColumn:3,textAlign:"center",fontSize:12.5,color:"#d1d5db"}}>│</div>
                <div style={{gridColumn:4,fontSize:12.5,color:"#4b5563"}}>{coSem}{coCv>0?<span style={{color:"#8b5cf6"}}>{` (+${coCv}?)`}</span>:""}</div>
              </React.Fragment>)})()}
            </div>
            {cfg.wp&&<div style={{marginTop:7,borderTop:"1px solid #e5e7eb",paddingTop:6,display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
              <div title={`Total LP toward the ${cfg.wp} LP Compulsory Electives target — past imported + this semester locked/likely across focus areas. Purple +N? = considering this semester.`} style={{textAlign:"center",padding:3,background:"#f9fafb",borderRadius:6,border:"1px solid #e5e7eb"}}>
                <div style={{fontSize:8.5,color:"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:".4px"}}>total</div>
                <div style={{fontWeight:800,fontSize:16,color:wpGrandTotal>=cfg.wp?"#059669":"#b45309"}}>{wpGrandTotal}{wpCons>0?<span style={{fontSize:11,color:"#8b5cf6"}}>{` +${wpCons}?`}</span>:""}</div>
                <div style={{fontSize:10,color:"#6b7280"}}>/ {cfg.wp} LP Compulsory Electives</div>
              </div>
              <div title="LP confirmed for this semester (locked + likely across all categories). Purple +N? = considering this semester." style={{textAlign:"center",padding:3,background:"#f9fafb",borderRadius:6,border:"1px solid #e5e7eb"}}>
                <div style={{fontSize:8.5,color:"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:".4px"}}>this sem</div>
          ...
          {(groupBy==="area"?categories:statusGroups).map(g=>{
            ...
            return(<div key={g.key} style={{marginBottom:4,background:"white",border:"1px solid #e5e7eb",borderRadius:7,overflow:"hidden"}}>
              ...
              {isOpen&&<div style={{padding:"2px 4px"}}>
                {filtW.map(c=>renderCourseRow(c,g))}
                ...
                {filtB.map(c=>renderCourseRow(c,g))}
              </div>}
            </div>);
          })}
              ...
              <React.Fragment key={h}>
                ...
                {visDays.map(di=>{
                  ...
                  return(<div key={`${di}-${h}`} style={{height:RH,borderTop:"1px solid #f0f0f0",borderLeft:"1px solid #f0f0f0",position:"relative"}}>
                    {starters.map(entry=>{
                      const c=entry.course,sl=entry.slot;
                      const sDec=entry.startDec,eDec=entry.endDec,dur=eDec-sDec;
                      const area=areaOf(c);const acl=getClForArea(area);
                      const isHard=conflicts.hardSet.has(c.id);
                      const isSoft=conflicts.softSet.has(c.id);
                      const hasConf=isHard||isSoft;
                      const isRem=presOf(c.id,entry.si)==="no";
                      ...
                      return(<div key={c.id+entry.si}
                        ...
                        onContextMenu={e=>{e.preventDefault();e.stopPropagation();setGridMenu(null);setPresMenu({id:c.id,si:entry.si,x:e.clientX,y:e.clientY})}}
                        onMouseEnter={()=>setHov(c.id)} onMouseMove={e=>setMousePos({x:e.clientX,y:e.clientY})} onMouseLeave={()=>setHov(null)} style={{
                        position:"absolute",top:(sDec-h)*RH,left:`${(idx/entries.length)*100}%`,width:`${100/entries.length}%`,
                        height:dur*RH-2,
        ...
        <div style={{marginTop:8,padding:"10px 14px",background:"#fef2f2",borderRadius:8,fontSize:13,color:"#991b1b",lineHeight:1.5}}>
          ...
          {fHardConf.map((p,i)=>(
            <div key={i} style={{marginBottom:1}}>
              {DAYS[p.day]} {fmtTimeShort(p.from)}–{fmtTimeShort(p.to)}: <strong>{p.a.short}</strong> vs <strong>{p.b.short}</strong>
      ...
      {presMenu&&(()=>{
        ...
        const sl=c.slots[presMenu.si];
        const slotLabel=sl?` (${DAYS[sl.day]} ${fmtTimeShort(sl.start)}–${fmtTimeShort(sl.end)}${sl.label?" "+sl.label:""})`:"";
        const mx=Math.min(presMenu.x,window.innerWidth-170);
        const my=Math.min(presMenu.y,window.innerHeight-140);
        const pick=(v)=>{setPres(p=>({...p,[`${presMenu.id}:${presMenu.si}`]:v}));setPresMenu(null)};
        const cur=presOf(presMenu.id,presMenu.si);
        return(<>
          <div onClick={()=>setPresMenu(null)} style={{position:"fixed",inset:0,zIndex:999}}/>
          <div style={{position:"fixed",left:mx,top:my,zIndex:1000,background:"white",borderRadius:8,padding:"4px",boxShadow:"0 4px 20px rgba(0,0,0,.15)",border:"1px solid #e5e7eb",minWidth:150}}>
            <div style={{padding:"4px 8px",fontSize:11,color:"#9ca3af",fontWeight:600,borderBottom:"1px solid #f3f4f6",marginBottom:2}}>{c.short}{slotLabel} — Attendance</div>
            {PR.map(p=>(
              <button key={p.v} onClick={()=>pick(p.v)} style={{display:"flex",alignItems:"center",gap:6,width:"100%",padding:"6px 8px",border:"none",background:cur===p.v?`${p.c}10`:"white",borderRadius:4,fontSize:13,color:"#374151",textAlign:"left",cursor:"pointer"}}>
            ...
            <div style={{borderTop:"1px solid #f3f4f6",margin:"4px 0 2px",padding:"4px 8px 4px"}}>
              <div style={{fontSize:10,color:"#9ca3af",fontWeight:600,marginBottom:2}}>Room</div>
              <input
                key={`${presMenu.id}:${presMenu.si}`}
                defaultValue={(sl&&sl.room)||""}
                placeholder={c.room||"Room"}
                onClick={e=>e.stopPropagation()}
                onBlur={e=>{updateSlotRoom(presMenu.id,presMenu.si,e.target.value);setPresMenu(null)}}
                onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();updateSlotRoom(presMenu.id,presMenu.si,e.target.value);setPresMenu(null)}else if(e.key==="Escape"){setPresMenu(null)}}}
                style={{width:"100%",padding:"3px 5px",borderRadius:4,border:"1px solid #d1d5db",fontSize:12,boxSizing:"border-box"}}
                title="Press Enter to save this slot's room"
              />
            </div>
          </div>
        </>);
      ...
      {/* ECTS bar segment tooltip (instant, no delay) */}
      {barTip&&(()=>{
        const tx=Math.min(barTip.x+12,window.innerWidth-220);
        const ty=Math.min(barTip.y+12,window.innerHeight-40);
        return(<div style={{position:"fixed",left:tx,top:ty,zIndex:1001,background:"#111827",color:"white",borderRadius:5,padding:"4px 8px",fontSize:11.5,fontWeight:500,pointerEvents:"none",boxShadow:"0 4px 12px rgba(0,0,0,.2)",whiteSpace:"nowrap"}}>{barTip.text}</div>);
      })()}
      ...
      {hov&&(()=>{const c=courses.find(x=>x.id===hov);if(!c)return null;const area=areaOf(c);const acl=getClForArea(area);const st=stOf(c.id);
        ...
        return(<div style={{position:"fixed",left:tx,top:ty,zIndex:1000,background:"white",borderRadius:10,padding:"10px 14px",fontSize:14,lineHeight:1.6,maxWidth:360,border:"1px solid #bfdbfe",boxShadow:"0 4px 20px rgba(0,0,0,.12)",pointerEvents:"none"}}>
          ...
          {c.slots.length>0&&<div style={{fontSize:12,color:"#6b7280",marginTop:2}}>{c.slots.map((s,i)=>`${DAYS[s.day]} ${fmtTime(s.start)}–${fmtTime(s.end)}${s.label?" ("+s.label+")":""}`).join(", ")}</div>}
          {hasPerSlotRooms?<div style={{fontSize:12,color:"#4b5563",marginTop:2}}>Rooms: {c.slots.map((s,i)=>`${DAYS[s.day]} ${fmtTimeShort(s.start)}–${fmtTimeShort(s.end)}: ${s.room||c.room||"?"}`).join(", ")}</div>
